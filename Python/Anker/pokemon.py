...
def sec(inv):
	headers = {
		'Referer': 'https://www.anker.com/register?back=deals%2Fpokessentials',
	...
	try:
		email = '{0}{1}{2}@mailinator.com'.format(random.choice(names).strip(), random.choice(names).strip(), random.randint(0,99))
		work = False
		...
		r = requests.post(
			"https://www.anker.com/api/content?path=/api/registrations",
			json={"register_source":"https%3A%2F%2Fwww.anker.com%2Fdeals%2Fpokessentials","email":email,"password":"playtime2","is_subscribe":false,"order_id":"","cachetime":"170153"},
			verify=False,
			timeout=6,
			proxies=prozy,
			headers=headers
