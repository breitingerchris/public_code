...
class App():
	def __init__(self, parent):
		...
		# Setup Window
		self.window = parent
		self.window.title('FurForce: Amazon')
	...
	def Login(self, user, password, proxie1s):
		...
		while work:
			...
			openidreturn_to = re.findall('<input type="hidden" name="openid.return_to" value="(.*?)" />',
										 amazon.content)
			context = re.findall('<input type="hidden" name="clientContext" value="(.*?)" />',
										 amazon.content)
			previd = re.findall('<input type="hidden" name="prevRID" value="(.*?)" />',
										 amazon.content)
			#state = re.findall('<input type="hidden" name="siteState" value="(.*?)" />',
			#							 amazon.content)
			#placeid = re.findall('<input type="hidden" name="marketPlaceId" value="(.*?)" />',
			#							 amazon.content)
			url = re.findall('ap_signin_form" novalidate="novalidate" action="(.*?)"',
										 amazon.content)
			pageid = re.findall('<input type="hidden" name="pageId" value="(.*?)" />',
										 amazon.content)
			max_auth_age = re.findall('<input type="hidden" name="openid.pape.max_auth_age" value="(.*?)" />',
										 amazon.content)
			data = {}
